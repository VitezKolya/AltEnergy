...
import altenergy.world.gen.layer.GenLayerTerra;
...
public class WorldChunkManagerTerra extends WorldChunkManager {

	private GenLayer terraGenBiomes;
	private GenLayer terraBiomeIndexLayer;
	private BiomeCache terraBiomeCache;
	private List<BiomeGenBase> terraBiomeToSpawnIn;
	...
	public WorldChunkManagerTerra(long seed, WorldType worldType) {

		this();
		GenLayer[] agenlayer = GenLayerTerra.makeTheWorld(seed);
		terraGenBiomes = agenlayer[0];
		terraBiomeIndexLayer = agenlayer[1];
	}

	public WorldChunkManagerTerra(World world) {
	...
	public List<BiomeGenBase> getBiomesToSpawnIn() {
		...
		return terraBiomeToSpawnIn;
	}

	/**
	 * Returns the BiomGenBase related to the x, z position on the world.
	 */
	@Override
	public BiomeGenBase getBiomeGenAt(int x, int z) {

		BiomeGenBase biome = terraBiomeCache.getBiomeGenAt(x, z);
		if (biome == null) {
			return AEBiomeList.terraOcean;
		}
	...
	public float[] getRainfall(float[] par1ArrayOfFloat, int par2, int par3, int par4, int par5) {
		...
		int[] aint = terraBiomeIndexLayer.getInts(par2, par3, par4, par5);
	...
	public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] par1ArrayOfBiomeGenBase, int par2, int par3, int par4,
		...
		int[] aint = terraGenBiomes.getInts(par2, par3, par4, par5);
		...
		for (int i = 0; i < par4 * par5; ++i) {
			if (aint[i] >= 0) {
				...
				par1ArrayOfBiomeGenBase[i] = AEBiomeList.terraOcean;
			}
		}
	...
	public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] par1ArrayOfBiomeGenBase, int x, int y, int width, int length,
		...
		if (cacheFlag && width == 16 && length == 16 && (x & 15) == 0 && (y & 15) == 0) {
			BiomeGenBase[] abiomegenbase1 = terraBiomeCache.getCachedBiomes(x, y);
			System.arraycopy(abiomegenbase1, 0, par1ArrayOfBiomeGenBase, 0, width * length);
			return par1ArrayOfBiomeGenBase;
		} else {
			int[] aint = terraBiomeIndexLayer.getInts(x, y, width, length);
	...
	public boolean areBiomesViable(int par1, int par2, int par3, List par4List) {
		...
		int l1 = j1 - l + 1;
		int i2 = k1 - i1 + 1;
		int[] aint = terraGenBiomes.getInts(l, i1, l1, i2);
