...
public class TutorialTeleporter extends Teleporter {
	 ...
	 * on the X & Z of the players initial
	 * location.
	 */
			...
			for (int l = -2; l <= 2; ++l) {
				for (int i1 = -2; i1 <= 2; ++i1) {
					for (int j1 = -1; j1 < 3; ++j1) {
						...
						/** change this block **/
						worldServerInstance.setBlock(k1, l1, i2, flag ? BlockIds.GRANITE : 0);
					}
				}
			}
			...
			for (k1 = l - short1; k1 <= l + short1; ++k1) {
				...
				for (int l1 = i1 - short1; l1 <= i1 + short1; ++l1) {
					...
					for (int i2 = worldServerInstance.getActualHeight() - 1; i2 >= 0; --i2) {
						/** change this block **/
						if (worldServerInstance.getBlockId(k1, i2, l1) == BlockIds.PORTAL) {
							/** change this block **/
							while (worldServerInstance.getBlockId(k1, i2 - 1, l1) == BlockIds.PORTAL) {
								--i2;
							}
		...
		if (d3 >= 0.0D) {
			if (flag) {
				destinationCoordinateCache.add(j1,
						new PortalPosition(this, i, j, k, worldServerInstance.getTotalWorldTime()));
			...
			int j2 = -1;
			/** change this block **/
			if (worldServerInstance.getBlockId(i - 1, j, k) == BlockIds.PORTAL) {
				j2 = 2;
			}
			/** change this block **/
			if (worldServerInstance.getBlockId(i + 1, j, k) == BlockIds.PORTAL) {
			...
			/** change this block **/
			if (worldServerInstance.getBlockId(i, j, k - 1) == BlockIds.PORTAL) {
				j2 = 3;
			}
			/** change this block **/
			if (worldServerInstance.getBlockId(i, j, k + 1) == BlockIds.PORTAL) {
							...
							for (j3 = 0; j3 < 3; ++j3) {
								for (i4 = 0; i4 < 4; ++i4) {
									for (l3 = -1; l3 < 4; ++l3) {
										...
										if (l3 < 0 && !worldServerInstance.getBlockMaterial(k4, j4, l4).isSolid()
							...
							for (i3 = l1; i3 < l1 + 2; ++i3) {
								...
								for (j3 = 0; j3 < 4; ++j3) {
									for (i4 = -1; i4 < 4; ++i4) {
										...
										k4 = k2 + i4;
										j4 = j2 + (j3 - 1) * k3;
										if (i4 < 0 && !worldServerInstance.getBlockMaterial(l3, k4, j4).isSolid()
			...
			for (k2 = -1; k2 <= 1; ++k2) {
				for (i3 = 1; i3 < 3; ++i3) {
					for (l2 = -1; l2 < 3; ++l2) {
						...
						/** change this block **/
						worldServerInstance.setBlock(k3, j3, i4, flag ? BlockIds.GRANITE : 0);
					}
				}
			}
		}
	...
	public void removeStalePortalLocations(long par1) {
		...
		if (par1 % 100L == 0L) {
			...
			while (iterator.hasNext()) {
				...
				PortalPosition portalposition = (PortalPosition) destinationCoordinateCache.getValueByKey(olong
						.longValue());
