...
public abstract class MultiblockStructureUtil {
	...
	/**
	 * The structure should be made in the master block, then inherited by the other blocks in the structure.
	 * @param masterX
	...
	public int[] getMasterBlockLocalCoords(int rot) {
		...
		switch(rot) {
			case 1:
				return new int[] { lenZ - 1 - z, y, x};
			case 2:
				return new int[] { lenX - 1 - x, y, z};
			case 3:
				return new int[] { z, y, lenX - 1 - x};
			default:
	...
	public boolean isValidStructure() {
		...
		for (int rotate = 0; rotate < 4; rotate++) {
			structure = this.getStructure(rotate, this.isValid ? this.getFinalStructure() : this.getInitializingStructure());
			
			for(int x = 0; x < structure.length; x++) {
				int xCoord = this.masterX -(this.getMasterBlockWorldCoords()[0] - x);
				
				for(int y = 0; y < structure[x].length; y++) {
					int yCoord = this.masterY - (this.getMasterBlockWorldCoords()[1] - y);
					
					for(int z = 0; z < structure[x][y].length; z++) {
						int zCoord = this.masterZ - (this.getMasterBlockWorldCoords()[2] - z);
	...
	public void removeStructureTiles() {
		...
		for(int x = 0; x < initStructure.length; x++) {
			int xCoord = this.masterX -(this.getMasterBlockLocalCoords(rotation)[0] - x);
			
			for(int y = 0; y < initStructure[x].length; y++) {
				int yCoord = this.masterY - (this.getMasterBlockLocalCoords(rotation)[1] - y);
				
				for(int z = 0; z < initStructure[x][y].length; z++) {
					int zCoord = this.masterZ - (this.getMasterBlockLocalCoords(rotation)[2] - z);
					
					if(xCoord == masterX && yCoord == masterY && zCoord == masterZ) {
						continue;
					}
					
					if(world.getBlockId(xCoord, yCoord, zCoord) != finalStructure[x][y][z] || finalStructure[x][y][z] == Integer.MIN_VALUE) {
						continue;
					}
