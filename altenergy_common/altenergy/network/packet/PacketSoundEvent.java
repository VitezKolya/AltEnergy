...
import altenergy.lib.Commands;
import altenergy.network.PacketTypeHandler;
import cpw.mods.fml.client.FMLClientHandler;
import cpw.mods.fml.common.network.Player;

public class PacketSoundEvent extends PacketAE {
	...
	public void execute(INetworkManager manager, Player player) {
		...
		if (ConfigurationSettings.ENABLE_SOUNDS.equalsIgnoreCase(Commands.ALL)) {
			FMLClientHandler.instance().getClient().sndManager.playSound(soundName, (float) x, (float) y, (float) z,
					volume, pitch);
			...
			if (thePlayer.username.equalsIgnoreCase(playerName)) {
				FMLClientHandler.instance().getClient().sndManager.playSound(soundName, (float) x, (float) y,
						(float) z, volume, pitch);
			}
		}
	}
}
